...
def teigi(a, b, c, prefix="0UL", suffix="")
  _a = [a].flatten.map{|k| "B_" + k}.join("|")
  sprintf("  {.shift = %-10s , .douji = %-15s , .kana = %-8s , .func = nofunc }, // #{c}", prefix, _a, "\"#{b}\"")
end
...
def outputHenshu(pk, m, k)
  ...
  $hcase << "}\n\n"
  $hkey << sprintf("  {.shift = %-10s , .douji = B_%-5s , .kana = \"\" , .func = ngh_%-6s }, // #{m}", pk, k, pk.gsub(/\|*B_/, "")+k)
end
